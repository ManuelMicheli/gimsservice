...
/**
 * I Nostri Servizi — schede wide impilate (sticky stacking).
 * Ogni scheda si "incolla" e la successiva la ricopre scorrendo, con parallax
 * sull'immagine e leggero rientro in scala della scheda sottostante (effetto White Maple).
 */
  ...
  return (
    <section id="servizi" className="shell py-24 md:py-32">
      ...
      <div className="mt-14 md:mt-20">
        {SERVICES.map((s, i) => (
          <ServiceCard key={s.n} service={s} index={i} total={SERVICES.length} />
        ))}
      </div>
    </section>
  );
...
function ServiceCard({
  ...
  // Parallax verticale sull'immagine.
  const y = useTransform(scrollYProgress, [0, 1], ["-8%", "8%"]);
  // La scheda rientra leggermente quando la successiva la ricopre.
  ...
  // top sticky incrementale → si vede il bordo delle schede impilate sotto.
    ...
    <div ref={ref} className="sticky" style={{ top: topPx }}>
      ...
      >
        ...
        <div className="flex shrink-0 flex-col gap-5 p-8 sm:p-10 md:flex-row md:items-end md:justify-between md:gap-12 md:p-14">
          <h3 className="font-display text-4xl font-light leading-[1.05] tracking-tight text-ink md:text-6xl 2xl:text-7xl">
            {service.title}
          </h3>
